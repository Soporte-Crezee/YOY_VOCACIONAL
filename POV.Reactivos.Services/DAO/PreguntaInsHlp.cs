   ...
   public class PreguntaInsHlp { 
      ...
      public void Action(IDataContext dctx, Pregunta pregunta, Reactivo reactivo){
         ...
         if (pregunta == null)
            sError += ", Pregunta";
         if (sError.Length > 0)
            throw new Exception("PreguntaInsHlp: Los siguientes campos no pueden ser vacios " + sError.Substring(2));
         if (reactivo == null)
            sError += ", Reactivo";
         if (sError.Length > 0)
            throw new Exception("PreguntaInsHlp: Los siguientes campos no pueden ser vacios " + sError.Substring(2));
      ...
      throw new StandardException(MessageType.Error, "", "DataContext no puede ser nulo", "POV.Reactivos.DAO", 
         "PreguntaInsHlp", "Action", null, null);
         DbCommand sqlCmd = null;
         try{
            dctx.OpenConnection(myFirm);
            sqlCmd = dctx.CreateCommand();
         } catch(Exception ex){
      throw new StandardException(MessageType.Error, "", "PreguntaInsHlp: No se pudo conectar a la base de datos", "POV.Reactivos.DAO", 
         "PreguntaInsHlp", "Action", null, null);
         }
         DbParameter sqlParam;
         StringBuilder sCmd = new StringBuilder();
         sCmd.Append(" INSERT INTO Pregunta (Orden, Descripcion,Valor, TextoPregunta, PlantillaPregunta, FechaRegistro, ReactivoID, Activo) ");
         sCmd.Append(" VALUES ");
         sCmd.Append(" ( ");
         // pregunta.Orden
         ...
         try{
            ...
            throw new Exception("PreguntaInsHlp: Ocurrió un error al ingresar el registro. " + exmsg);
         } finally{
            try{ dctx.CloseConnection(myFirm); } catch(Exception){ }
         }
         if (iRes < 1)
            throw new Exception("PreguntaInsHlp: Ocurrió un error al ingresar el registro.");
      }
   } 
}
