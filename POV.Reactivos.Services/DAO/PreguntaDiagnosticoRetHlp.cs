...
namespace POV.Reactivos.DAO { 
   /// <summary>
   /// Consulta un registro de Pregunta (Diagnostico) en la BD
   /// </summary>
   public class PreguntaDiagnosticoRetHlp { 
      /// <summary>
      ...
      public DataSet Action(IDataContext dctx, Pregunta pregunta, Reactivo reactivo){
         ...
         if (pregunta == null)
            sError += ", Pregunta";
         if (sError.Length > 0)
            throw new Exception("PreguntaDiagnosticoRetHlp: Los siguientes campos no pueden ser vacios " + sError.Substring(2));
      if (reactivo == null) {
         reactivo = new Reactivo();
      }
         if (dctx == null)
      throw new StandardException(MessageType.Error, "", "DataContext no puede ser nulo", "POV.Reactivos.DAO", 
         "PreguntaDiagnosticoRetHlp", "Action", null, null);
         DbCommand sqlCmd = null;
         try{
            dctx.OpenConnection(myFirm);
            sqlCmd = dctx.CreateCommand();
         } catch(Exception ex){
      throw new StandardException(MessageType.Error, "", "PreguntaDiagnosticoRetHlp: No se pudo conectar a la base de datos", "POV.Reactivos.DAO", 
         "PreguntaDiagnosticoRetHlp", "Action", null, null);
         }
         DbParameter sqlParam;
         StringBuilder sCmd = new StringBuilder();
         sCmd.Append(" SELECT PreguntaID, ReactivoID, Orden, Descripcion='', TextoPregunta, Valor, PlantillaPregunta, FechaRegistro, PuedeOmitir,  Activo, SoloImagen ");
         sCmd.Append(" FROM PreguntaDiagnostico ");
         ...
         if (pregunta.TextoPregunta != null){
            s_VarWHERE.Append(" AND TextoPregunta = @dbp4ram3 ");
            sqlParam = sqlCmd.CreateParameter();
         ...
         if (pregunta.Valor != null){
            s_VarWHERE.Append(" AND Valor = @dbp4ram4 ");
            sqlParam = sqlCmd.CreateParameter();
         ...
         if (pregunta.PlantillaPregunta != null){
            s_VarWHERE.Append(" AND PlantillaPregunta = @dbp4ram5 ");
            sqlParam = sqlCmd.CreateParameter();
         ...
         {
             s_VarWHERE.Append(" AND SoloImagen = @dbp4ram10 ");
             sqlParam = sqlCmd.CreateParameter();
         ...
         try{
            ...
            throw new Exception("PreguntaDiagnosticoRetHlp: Se encontraron problemas al recuperar los datos. " + exmsg);
         } finally{
            try{ dctx.CloseConnection(myFirm); } catch(Exception){ }
         }
         return ds;
