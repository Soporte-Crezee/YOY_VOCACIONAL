...
{
    public class PoliticaEscalaPuntaje : APoliticaEscalaDinamica
    {
        ...
        {            
			...
			if (nuevo.PuntajeMinimo >= nuevo.PuntajeMaximo)
				throw new Exception(String.Format(
					"La puntuación inicial no puede ser mayor a la puntuación final:\n\n Rango: {0} - {1}", 
			...
			if (!pruebaDinamica.ListaPuntajes.Any())
			{
				...
				if (nuevo.PuntajeMinimo == 0)
					...
					throw new Exception("El primer rango de la prueba, debe tener como puntuación inicial cero");
			}
            ...
            {
                AEscalaDinamica escala = ap as EscalaPuntajeDinamica;
