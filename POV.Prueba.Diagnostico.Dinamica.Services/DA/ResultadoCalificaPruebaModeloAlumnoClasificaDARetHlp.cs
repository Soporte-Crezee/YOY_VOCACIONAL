...
{
    class ResultadoCalificaPruebaModeloAlumnoClasificaDARetHlp
    {
        /// <summary>
        ...
        public DataSet Action(IDataContext dctx, Escuela escuela, GrupoCicloEscolar grupoCicloEscolar, PruebaDinamica prueba, Alumno alumno)
        {
            ...
            if (prueba == null || prueba.PruebaID == null)
                sError += ", PruebaID";
            if (sError.Length > 0)
                throw new Exception("ResultadoCalificaPruebaModeloAlumnoClasificaDARetHlp: Los siguientes campos no pueden ser vacios " + sError.Substring(2));

            if (dctx == null)
                throw new Exception("ResultadoCalificaPruebaModeloAlumnoClasificaDARetHlp.Action: DataContext no puede ser nulo");
            ...
            {
                throw new Exception("ResultadoCalificaPruebaModeloAlumnoClasificaDARetHlp.Action: Hubo un error al conectarse a la Base de Datos\n " + ex.Message);
            }
            ...
            sCmd.Append(@" SELECT DISTINCT ResultadoPruebaID, AlumnoID, GrupoCicloEscolarID, CicloEscolarID, 
                            ...
                            DetResulC_Valor, DetResulC_EsAproximado, 
                            EscalaDC_PuntajeMinimo, EscalaDC_PuntajeMaximo, EscalaDC_EsPorcentaje, EscalaDC_EsPredominante, 
            ...
            try
            {
                ...
                sqlAdapter.Fill(ds, "ResultadoCalificaClasificacion");
            }
            catch (Exception ex)
            {
                ...
                throw new Exception("ResultadoCalificaPruebaModeloAlumnoClasificaDARetHlp: Se encontraron problemas al recuperar los datos. " + exmsg);
            }
            finally
            {
                try { dctx.CloseConnection(myFirm); }
