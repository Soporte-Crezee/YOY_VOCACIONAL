...
{
    internal class PruebasAlumnosDARetHlp
    {
        /// <summary>
        ...
        public DataSet Action(IDataContext dctx, Alumno alumno, APrueba prueba, String nombreCompletoAlumno = "")
        {
            object myFirm = new object();
            string sError = String.Empty;
            if (prueba == null)
            ...
            sCmd.Append("SELECT a.AlumnoID, a.Nombre+' '+PrimerApellido+' '+SegundoApellido NombreAlumno, p.Nombre NombrePrueba,  ");
            sCmd.Append("       rpd1.FechaInicio, rpd1.FechaFin ");
            sCmd.Append("  FROM [Alumno] a ");
            sCmd.Append("  inner join [ResultadoPruebaDinamica] rpd on rpd.AlumnoID = a.AlumnoID ");
            sCmd.Append("  inner join [ResultadoPrueba] rp on rp.ResultadoPruebaID = rpd.ResultadoPruebaID ");
            sCmd.Append("  inner join [Prueba] p on p.PruebaID = rp.PruebaID ");
            sCmd.Append("  inner join [RegistroPruebaDinamica] rpd1 on rpd1.ResultadoPruebaID = rpd.ResultadoPruebaID ");
            ...
            if (alumno.AlumnoID != null)
            {
                s_VarWHERE.Append(" AND a.AlumnoID = @dbp4ram4 ");
                sqlParam = sqlCmd.CreateParameter();
                sqlParam.ParameterName = "dbp4ram4";
        ...
        /// <summary>
        /// Consulta registros de AResultadoPrueba de la bateria de Bullying en la base de datos.
        /// </summary>
        /// <param name="dctx">El DataContext que proveerá acceso a la base de datos</param>
        /// <param name="usuario"> Usuario que provee el criterio de selección para realizar la consulta</param>
        /// <param name="pruebas"> Lista de pruebas de bateria de bullying que provee el criterio de selección para realizar la consulta</param>
        /// <param name="nombreCompletoAlumno"> Nombre del Estudiante que provee el criterio de selección para realizar la consulta</param>
        /// <returns></returns>
        public DataSet Action(IDataContext dctx, Usuario usuario, string pruebas, String nombreCompletoAlumno = "")
        {
            object myFirm = new object();
            string sError = String.Empty;
            if (string.IsNullOrEmpty(pruebas))
                sError += ", Pruebas";
            if (usuario == null)
                sError += ", Alumno";
            if (sError.Length > 0)
            ...
            DbParameter sqlParam;
            StringBuilder sCmd = new StringBuilder();
            sCmd.Append("SELECT result.AlumnoID, result.NombreAlumno, result.Pruebas ");
            sCmd.Append("from(  ");
            sCmd.Append("SELECT a.AlumnoID, a.Nombre+' '+PrimerApellido+' '+SegundoApellido NombreAlumno, COUNT_BIG(*) AS Pruebas ");
            //sCmd.Append("       rpd1.FechaInicio, rpd1.FechaFin ");
            sCmd.Append("  FROM [UsuarioExpediente] ue ");
            sCmd.Append("  inner join [Alumno] a on a.AlumnoID = ue.AlumnoID ");
            sCmd.Append("  inner join [ResultadoPruebaDinamica] rpd on rpd.AlumnoID = ue.AlumnoID ");
            ...
            //if (prueba.PruebaID != null)
            //{
            s_VarWHERE.Append(" rp.PruebaID IN(" + pruebas + ")");
            //    sqlParam = sqlCmd.CreateParameter();
            //    sqlParam.ParameterName = "dbp4ram1";
            //    sqlParam.Value = prueba.PruebaID;
            //    sqlParam.DbType = DbType.Int32;
            ...
            sCmd.Append("  group by a.alumnoid, a.Nombre, a.PrimerApellido, a.segundoapellido) as result ");
            sCmd.Append("  WHERE result.Pruebas = 12 ");
