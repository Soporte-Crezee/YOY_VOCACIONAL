...
{
    internal class ResultadoPruebaFrasesVocacionalesDARetHlp
    {
        /// <summary>
        ...
        public DataSet Action(IDataContext dctx, Alumno alumno, APrueba prueba)
        {
            ...
            if (alumno == null)
                sError += ", Alumno";
            if (sError.Length > 0)
                throw new Exception("ResultadoPruebaFrasesVocacionalesDARetHlp: Los siguientes campos no pueden ser vacios " + sError.Substring(2));
            ...
            if (dctx == null)
                throw new StandardException(MessageType.Error, "", "DataContext no puede ser nulo", "POV.PruebaDiagnostico.DA",
                   "ResultadoPruebaFrasesVocacionalesDARetHlp", "Action", null, null);
            ...
            {
                throw new StandardException(MessageType.Error, "", "ResultadoPruebaFrasesVocacionalesDARetHlp: No se pudo conectar a la base de datos", "POV.PruebaDiagnostico.DA",
                   "ResultadoPruebaFrasesVocacionalesDARetHlp", "Action", null, null);
            }
            ...
            catch (Exception ex)
            {
                ...
                throw new Exception("ResultadoPruebaFrasesVocacionalesDARetHlp: Se encontraron problemas al recuperar los datos. " + exmsg);
            }
            finally
            {
                try { dctx.CloseConnection(myFirm); }
