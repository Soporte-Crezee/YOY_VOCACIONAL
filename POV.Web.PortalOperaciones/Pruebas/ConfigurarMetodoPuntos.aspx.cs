...
{
	public partial class ConfigurarMetodoPuntos : PageBase
	{
            ...
            try
            {
                if (!IsPostBack)
                {
                    if (LastObject != null && LastObject.PruebaID != null)
                    {
                        ...
                        {
							if ((prueba.Modelo as ModeloDinamico).MetodoCalificacion == EMetodoCalificacion.PUNTOS)
							{
								LastObject = prueba;
		...
		protected void BtnAgregarRango_Click(object sender, EventArgs e)
		{
			if (this.grdRangos.EditIndex < 0)
			{
				...
				if (string.IsNullOrEmpty(sError))
				{
					...
					try
					{
						PoliticaEscalaPuntaje politica = new PoliticaEscalaPuntaje();
						if (politica.Validar(new PruebaDinamica(this.ListaEscalasDinamicas), escala))
						{
							...
							//Limpiar campos
							this.txtPuntajeInicial.Text = "";
							this.txtPuntajeFinal.Text = "";
						...
						else
						{
							...
							"Los rangos no deben traslaparse, ni existir conjuntos de puntajes no asignados entre dos rangos.\n\n rango: {0} - {1}",
			...
			if (this.grdRangos.EditIndex < 0)
			{
				...
				try
				{
					if (pruebaDinamica.ListaPuntajes.Where(x => x.EsPredominante == true).Count() != 1)
						throw new Exception("Es necesario que exista un único rango predominante, por favor verifique");
		...
		protected void grdRangos_RowUpdating(object sender, GridViewUpdateEventArgs e)
		{
			...
			else if (puntajeMinimo.Length > 11)
				sError += ", rango inicial no puede tener más 11 caracteres ";
			...
			else if (puntajeMaximo.Length > 11)
				sError += ", rango final no puede tener más de 11 caracteres ";
			...
			if (string.IsNullOrEmpty(sError))
			{
				...
				escalaValidar = new EscalaPuntajeDinamica();
		...
		private AEscalaDinamica GetEscalaDinamicaFromUI()
		{
			...
			ruta = new EscalaPuntajeDinamica();

			ruta.PuntajeMinimo = decimal.Parse(this.txtPuntajeInicial.Text);
			ruta.PuntajeMaximo = decimal.Parse(this.txtPuntajeFinal.Text);
		...
		private string ValidarEscala()
		{
			...
			decimal valorMin = 0;
			if (string.IsNullOrEmpty(this.txtPuntajeInicial.Text))
				sError += ", puntuación inicial es requerido ";
			else if (this.txtPuntajeInicial.Text.Length > 11)
				sError += ", puntuación inicial no puede tener más de 11 caracteres ";
			else if (!decimal.TryParse(this.txtPuntajeInicial.Text, out valorMin))
				sError += ", puntuación inicial debe ser numérico ";
			else if (valorMin < 0)
			{
				sError += ", puntuación inicial no puede ser menor a cero";
			}

			decimal valorMax = 0;
			if (string.IsNullOrEmpty(this.txtPuntajeFinal.Text))
				sError += ", puntuación final es requerido ";
			else if (this.txtPuntajeFinal.Text.Length > 11)
				sError += ", puntuación final no puede tener más de 11 caracteres ";
			else if (!decimal.TryParse(this.txtPuntajeFinal.Text, out valorMax))
				sError += ", puntuación final debe ser numérico ";
			else if (valorMax < 0)
			{
				sError += ", puntuación final no puede ser menor a cero";
			}

			if (string.IsNullOrEmpty(sError) && valorMin >= valorMax)
				sError += ", puntuación inicial no puede ser mayor o igual a puntuación final ";
