...
namespace POV.Web.PortalOperaciones.Cargador
{
    public partial class CargaTutores : CatalogPage
    {
        private CargarTutorCtrl cargarTutorCtrl;
        ...
        {
            get { return (DataTable)this.Session["ResultadoCargaTutores"]; }
            set { this.Session["ResultadoCargaTutores"] = value; }
        }
        private DataTable SS_ListadoCarga
        {
            get { return (DataTable)this.Session["ListadoCargaTutores"]; }
            set { this.Session["ListadoCargaTutores"] = value; }
        }
        #endregion
        ...
        public CargaTutores()
        {
            ...
            cargarTutorCtrl = new CargarTutorCtrl();
        }
        ...
        protected void btnCargar_OnClick(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(fuArchivoTutores.PostedFile.FileName))
                this.ShowMessage("Es requerido seleccionar la ubicación del archivo a cargar.", MessageType.Warning);

            if (!fuArchivoTutores.HasFile || !fuArchivoTutores.FileName.EndsWith("xls", true, CultureInfo.InvariantCulture))
                if (!fuArchivoTutores.FileName.EndsWith("xlsx", true, CultureInfo.InvariantCulture))
            ...
            try
            {
                ...
                strpath = MapPath("~/Content/Temp/") + string.Format("{0}{1}", DateTime.Now.Ticks, fuArchivoTutores.FileName);
                ScriptManager.RegisterStartupScript(this, this.GetType(), "script", "alert('grabando archivo en: '"+strpath+"');", true);
                fuArchivoTutores.SaveAs(strpath);
                System.Threading.Thread.Sleep(10000);
                ScriptManager.RegisterStartupScript(this, this.GetType(), "script", "alert('Se continuara el registro de los padres');", true);

                this.SS_ResultadoCarga = cargarTutorCtrl.CargarArchivoTutor(ConnectionHlp.Default.Connection, strpath);
                this.SS_ListadoCarga = cargarTutorCtrl.DsTutores.Tables[0];

                this.Response.Redirect("~/Reportes/CargaTutoresReport.aspx", false);
            }
