<%@ Page Title="" MasterPageFile="~/Site.Master" Language="C#" AutoEventWireup="true" CodeBehind="NuevoProducto.aspx.cs" Inherits="POV.Web.Administracion.Productos.NuevoProducto" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <script type="text/javascript">
        ...
        function habilitar(){       
            $("#<%=DDLTipoProducto.ClientID%>").change(function(){
                var valor=$("#<%=DDLTipoProducto.ClientID%>").val();
                if(valor!=1 ){
                    $("#<%=ddlPruebas.ClientID%>").removeClass('error');
                    $("#<%=ddlPruebas.ClientID%>").prop('required', false);
                    $("#<%=ddlPruebas.ClientID%>").attr("disabled", true);
                    $("#<%=ddlPruebas.ClientID%>").val('');                    
                }else
                {
                    $("#<%=ddlPruebas.ClientID%>").removeAttr('disabled');
                    $("#<%=ddlPruebas.ClientID%>").attr('required', true);
                }
            });       
        }
    </script>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <h3 class="ui-widget-header">
        ...
        /
        <asp:Label ID="lblAccion" runat="server" Text="Registrar"></asp:Label>
        producto
    </h3>
