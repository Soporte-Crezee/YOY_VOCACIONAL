    ...
    public class UbicacionRetHlp
    {
        ...
        public DataSet Action(IDataContext dctx, Ubicacion ubicacion)
        {
            ...
            if (ubicacion == null)
                sError += ", Ubicacion";
            if (sError.Length > 0)
                throw new Exception("UbicacionRetHlp: Los siguientes campos no pueden ser vacios " + sError.Substring(2));
            if (ubicacion.Pais == null)
                sError += ", Pais";
            if (ubicacion.Estado == null)
                sError += ", Estado";
            ...
            if (sError.Length > 0)
                throw new Exception("UbicacionRetHlp: Los siguientes campos no pueden ser vacios " + sError.Substring(2));
            if (dctx == null)
                throw new StandardException(MessageType.Error, "", "DataContext no puede ser nulo", "POV.Localizacion.DAO", "UbicacionRetHlp", "Action", null, null);
            ...
            {
                throw new StandardException(MessageType.Error, "", "UbicacionRetHlp: No se pudo conectar a la base de datos", "POV.Localizacion.DAO", "UbicacionRetHlp", "Action", null, null);
            }
            ...
            {
                s_Var.Append(" UbicacionID = @ubicacion_UbicacionID ");
                sqlParam = sqlCmd.CreateParameter();
                sqlParam.ParameterName = "ubicacion_UbicacionID";
                sqlParam.Value = ubicacion.UbicacionID;
            ...
            {
                s_Var.Append(" AND FechaRegistro= @ubicacion_FechaRegistro ");
                sqlParam = sqlCmd.CreateParameter();
            ...
            catch (Exception ex)
            {
                ...
                throw new Exception("UbicacionRetHlp: Se encontraron problemas al recuperar los datos. " + exmsg);
            }
            finally
            {
                try { dctx.CloseConnection(myFirm); }
