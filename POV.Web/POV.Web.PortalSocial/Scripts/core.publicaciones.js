...
//Para publicar en muro de un compañer@
function compartir() {
    ...
    if (contenido != '') {
        ...
        myApiBlockUI.send();
        api.Share({ success: function (result) { $("#txtPublicacion").css("height", "54px"); showPublicacion(result.d); myApiBlockUI.unblockContainer(); } }, dataString);
    }
}
...
function getOption(id, option) {
    if (option == 'delcom') {
        return "<input type=\"button\" id=\"btnDelete\" title=\"Eliminar Comentario\" onclick=\"javascript:removeComentario('" + id + "');\" class=\"operDel icon icon_cross floatRight\"/>"
    } else if (option == 'delpub') {
        return "<input type=\"button\" id=\"btnDelete\" title=\"Eliminar Publicación\" onclick=\"javascript:removePublicacion('" + id + "');\" class=\"operDel icon icon_cross floatRight\"/>";
    } else {
        return '';
    }
}
    ...
    if (data.d.length <= 0) {
        ...
        for (var x = 0; x < data.d.length; x++) {
            ...
            if (!pub) {
                ...
                var html_pub = $("#pubTmpl").tmpl(data.d[x], {
                    getTipoTexto: getTipoTexto,
                    urlFiles: getFilesUrl
                }).html();
        ...
        $('button[id|="btn-reportar"]').each(function (index) {
            $(this).button({
                icons: { primary: "ui-icon-flag" }, text: false
            });
        });
        ...
        if (data.d.length < 10) {
            ...
            if (place == MURO)
                $("#more").html('<a class="link_blue pconfirmacorreo" onclick="javascript:LoadPubs();" >Mostrar m&aacute;s publicaciones</a>');
            else
                $("#more").html('<a class="link_blue pconfirmacorreo" onclick="javascript:LoadPubsSocial();" >Mostrar m&aacute;s publicaciones</a>');
...
function showPublicacion(data) {
    $("#pubTmpl").tmpl(data, {
        getOption: getOption, getTipoTexto: getTipoTexto,
        urlFiles: getFilesUrl
...
function commentenable(pubid) {
    ...
    if (typeof TIPO_FUENTE_PUBLICACION != 'undefined' && TIPO_FUENTE_PUBLICACION.val() == "D")
    ...
    var options = {
        ...
        'originalStyle': 'display_info_textarea col-xs-12 pull-right',
        'warningStyle': 'display_warning_textarea col-xs-12 pull-right',
        'warningNumber': 200,
...
function sendcomment(pubid, commentdata) {
    ...
    contenido = $.trim(contenido);
    if ((contenido != '' || identificadorCompartir != undefined) && pubid != '') {
        var tipoContenido = $("#hdn-tipo-contenido-com").val();
        var tipo = identificadorCompartir != null && identificadorCompartir != '' ? 3 : 1;
...
function RefreshPublication(data, index) {
    ...
    if (index > size_li) {
        ...
        var pub = document.getElementById(data.publicacionid);
        if (!liItem) {
            var html_pub = $("#pubTmpl").tmpl(data, {
                getTipoTexto: getTipoTexto,
                urlFiles: getFilesUrl
            }).html();
    ...
    if (publicacionid != null && publicacionid != '') {
        ...
        api.GetPub({
            success: function (result) {
                ...
                $("#" + publicacionid).html("");
                var pubcomplete = $("#pubTmpl").tmpl(result.d, {
                    getTipoTexto: getTipoTexto,
                    urlFiles: getFilesUrl
                }).html();
...
function loadPublicacionComplete() {
    ...
    if (publicacionID.val() != null && publicacionID.val() != '') {
        ...
        if (place == VIEW_PUB) {
            var htm = '<p style="text-align:center">No se encontro la publicacion seleccionada</p>';
            $('#PublicacionStream').empty();
            $('#PublicacionStream').html(htm);
        }
...
function getFilesUrl() {
    var randomnumber = 2; // Math.floor(Math.random() * 10000)
...
function reportarAbusoPublicacionFromServer(reportableid) {
    ...
    var dataString = $.toJSON(data);
    myApiBlockUI.send();
    oReporteAbuso.AddReporteAbuso({
        success: function (result) { removeElementoReportadaFromUI(reportableid); myApiBlockUI.unblockContainer(); }, error: function (result) {
            myApiBlockUI.unblockContainer();
            $(hdnmessageinputid).val(data.d.error);
            $(hdnmessagetypeinputid).val('1');
            mostrarError();
...
function reportarAbusoComentario(reportableid) {
    var oReporteAbuso = new ReporteAbusoApi();
    myApiBlockUI.process();
    oReporteAbuso.validateReportarAbuso({
        success: function (result) {
