	...
	$[pluginName] = function (userOptions) {
		...
		var defaultOptions = {
			...
			buttons: {
				...
				prev: {
				    element: $("<input type='button' id='prev' value='Atras' />"),
				...
				if ($(step.selector).length > 0) {
					...
					scrollToElement(function () {
					    ...
					    if (plugin.options.steps[index - 1]) {
                            //add button prev
					        button = plugin.options.buttons.prev.element.clone();
					        button.addClass(plugin.options.buttons.prev.class).addClass(uniq);
					    ...
					    currentElement.on('shown.bs.popover', function () {
					        ...
					        $("#finish").on("click", function () {
					            plugin.finish();
					            __doPostBack('btnActualizarEstatusIdentificacion', '');
					        });
					    });
					});
				} else {
