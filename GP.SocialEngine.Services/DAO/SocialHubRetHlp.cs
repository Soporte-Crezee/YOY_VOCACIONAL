...
namespace GP.SocialEngine.DAO { 
   ...
   public class SocialHubRetHlp { 
      ...
      throw new StandardException(MessageType.Error, "", "DataContext no puede ser nulo", "GP.SocialEngine.DAO", 
         "SocialHubRetHlp", "Action", null, null);
         DbCommand sqlCmd = null;
         try{
            dctx.OpenConnection(myFirm);
            sqlCmd = dctx.CreateCommand();
         } catch(Exception ex){
      throw new StandardException(MessageType.Error, "", "SocialHubRetHlp: No se pudo conectar a la base de datos", "GP.SocialEngine.DAO", 
         "SocialHubRetHlp", "Action", null, null);
         }
         DbParameter sqlParam;
         StringBuilder sCmd = new StringBuilder();
         sCmd.Append(" SELECT SocialHubID, FechaRegistro, Alias, UsuarioSocialID,ReactivoID, InformacionSocialID,SocialProfileType, JuegoID ");
         ...
         {
             s_VarWHERE.Append(" AND SocialProfileType= @dbp4ram2 ");
             sqlParam = sqlCmd.CreateParameter();
         ...
         {
             if (socialHub.SocialProfile is UsuarioSocial && (socialHub.SocialProfile as UsuarioSocial).UsuarioSocialID != null)
             {
                 s_VarWHERE.Append(" AND UsuarioSocialID = @dbp4ram1 ");
                 sqlParam = sqlCmd.CreateParameter();
             ...
             else if (socialHub.SocialProfile is Reactivo && (socialHub.SocialProfile as Reactivo).ReactivoID != null)
             {
                 s_VarWHERE.Append(" AND ReactivoID = @dbp4ram1 ");
                 sqlParam = sqlCmd.CreateParameter();
                 sqlParam.ParameterName = "dbp4ram1";
                 sqlParam.Value = (socialHub.SocialProfile as Reactivo).ReactivoID;
