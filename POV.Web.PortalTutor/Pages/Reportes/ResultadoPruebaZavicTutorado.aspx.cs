...
using POV.Prueba.Diagnostico.Service;using Framework.Base.Exceptions;
using POV.CentroEducativo.BO;
...
{
    public partial class ResultadoPruebaZavicTutorado : System.Web.UI.Page
    {
        ...
        public ResultadoPruebaZavicTutorado()
        {
                ...
                case "VerReporte":
                    {
                        try
                        {
                            Response.Redirect("ReporteAlumnoResultadoZavic.aspx?num=" + e.CommandArgument.ToString());
                        }
                        catch (Exception ex)
                        {
        ...
        {
            LoadAlumnosZavic(new Tutor { TutorID = userSession.CurrentTutor.TutorID }, new PruebaDinamica { PruebaID = 21 }, "%" + txtNombre.Text.Trim() + "%");

        }

        private void LoadAlumnosZavic(Tutor tutor, APrueba prueba, String strNombre)
        {
            resultCtrl = new ResultadoPruebaDinamicaCtrl();
            grdAlumnosZavic.DataSource = resultCtrl.RetrieveAlumnosPruebas(ConnectionHlp.Default.Connection, tutor, prueba, strNombre);
