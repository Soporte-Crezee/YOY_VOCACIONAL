...
{
    public partial class ResultadoPruebaKuderTutorado : System.Web.UI.Page
    {
        ...
        public ResultadoPruebaKuderTutorado()
        {
                ...
                case "VerReporte":
                    {
                        try
                        {
                            Response.Redirect("ReporteAlumnoResultadoKuder.aspx?num=" + e.CommandArgument.ToString());
                        }
                        catch (Exception ex)
                        {
        ...
        {
            LoadAlumnosKuder(new Tutor { TutorID = userSession.CurrentTutor.TutorID }, new PruebaDinamica { PruebaID = 13 }, "%" + txtNombre.Text.Trim() + "%");
        }

        private void LoadAlumnosKuder(Tutor tutor, APrueba prueba, String strNombre)
        {
            resultCtrl = new ResultadoPruebaDinamicaCtrl();
            grdAlumnosKuder.DataSource = resultCtrl.RetrieveAlumnosPruebas(ConnectionHlp.Default.Connection, tutor, prueba, strNombre);
