...
{
    /// <summary>
    /// LicenciaDirectorRetHlp
    /// </summary>
    public class LicenciaDirectorRetHlp
    {
        /// <summary>
        /// Consulta registros de licenciaDirector en la base de datos.
        /// </summary>
        /// <param name="dctx">El DataContext que proveerá acceso a la base de datos</param>
        /// <param name="licenciaDirector">licenciaDirector que provee el criterio de selección para realizar la consulta</param>
        /// <returns>El DataSet que contiene la información de licenciaDirector generada por la consulta</returns>
        public DataSet Action(IDataContext dctx, LicenciaDirector licenciaDirector, LicenciaEscuela licenciaEscuela)
        {
            ...
            if (sError.Length > 0)
                throw new Exception("LicenciaDirectorRetHlp: Los siguientes campos no pueden ser vacíos: " + sError.Substring(2));
            if (licenciaEscuela.LicenciaEscuelaID == null)
                sError += ", LicenciaEscuela.LicenciaEscuelaID";
            if (sError.Length > 0)
                throw new Exception("LicenciaDirectorRetHlp: Los siguientes campos no pueden ser vacíos: " + sError.Substring(2));
            if (dctx == null)
                throw new StandardException(MessageType.Error, "", "DataContext no puede ser nulo", "POV.Licencias.DAO", "LicenciaDirectorRetHlp", "Action", null, null);
            ...
            {
                throw new StandardException(MessageType.Error, "", "LicenciaDirectorRetHlp: Hubo un error al conectarse a la base de datos", "POV.Licencias.DAO", "LicenciaDirectorRetHlp", "Action", null, null);
            }
            DbParameter sqlParam;
            StringBuilder sCmd = new StringBuilder();
            sCmd.Append(" SELECT lic.LicenciaID, LicenciaEscuelaID, UsuarioID, UsuarioSocialID, DirectorID, Tipo, Activo ");
            ...
            {
                s_VarWHERE.Append(" AND licRef.DirectorID =@dbp4ram4 ");
                sqlParam = sqlCmd.CreateParameter();
            ...
            try
            {
                ...
                sqlAdapter.Fill(ds, "LicenciaDirector");
            }
            catch (Exception ex)
            {
                ...
                throw new Exception("LicenciaDirectorRetHlp: Hubo un error al consultar los registros. " + exmsg);
            }
            finally
            {
                try { dctx.CloseConnection(myFirm); }
