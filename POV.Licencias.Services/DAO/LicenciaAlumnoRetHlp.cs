...
namespace POV.Licencias.DAO
{
    ...
    /// </summary>
    internal class LicenciaAlumnoRetHlp
    {
        /// <summary>
        /// Consulta registros de LicenciaAlumno en la base de datos.
        /// </summary>
        /// <param name="dctx">El DataContext que proveerá acceso a la base de datos</param>
        /// <param name="licenciaAlumno">LicenciaAlumno que provee el criterio de selección para realizar la consulta</param>
        /// <returns>El DataSet que contiene la información de LicenciaAlumno generada por la consulta</returns>
        public DataSet Action(IDataContext dctx, LicenciaAlumno licenciaAlumno, LicenciaEscuela licenciaEscuela)
        {
            ...
            if (sError.Length > 0)
                throw new Exception("LicenciaAlumnoRetHlp: Los siguientes campos no pueden ser vacíos: " + sError.Substring(2));
            if (licenciaEscuela.LicenciaEscuelaID == null)
                sError += ", LicenciaEscuela.LicenciaEscuelaID";
            if (sError.Length > 0)
                throw new Exception("LicenciaAlumnoRetHlp: Los siguientes campos no pueden ser vacíos: " + sError.Substring(2));
            if (dctx == null)
                throw new StandardException(MessageType.Error, "", "DataContext no puede ser nulo", "POV.Licencias.DAO", "LicenciaAlumnoRetHlp", "Action", null, null);
            ...
            sCmd.Append(" SELECT lic.LicenciaID, LicenciaEscuelaID, UsuarioID, UsuarioSocialID, AlumnoID, Tipo, Activo ");
            ...
            {
                s_VarWHERE.Append(" AND licRef.AlumnoID =@dbp4ram5 ");
                sqlParam = sqlCmd.CreateParameter();
            ...
            try
            {
                ...
                sqlAdapter.Fill(ds, "LicenciaAlumno");
            }
            catch (Exception ex)
            {
                ...
                throw new Exception("LicenciaAlumnoRetHlp: Hubo un error al consultar los registros. " + exmsg);
            }
            finally
            {
                try { dctx.CloseConnection(myFirm); }
