...
namespace POV.Licencias.DA { 
   /// <summary>
   /// Consulta un registro de Docente en la licencia que le corresponde en la BD
   /// </summary>
   public class DocenteUsuarioSocialDARetHlp { 
      /// <summary>
      ...
      public DataSet Action(IDataContext dctx, UsuarioSocial usuarioSocial){
         ...
         if (sError.Length > 0)
            throw new Exception("DocenteUsuarioSocialDARetHlp: Los siguientes campos no pueden ser vacios " + sError.Substring(2));
      ...
      throw new StandardException(MessageType.Error, "", "DataContext no puede ser nulo", "POV.Licencias.DA", 
         "DocenteUsuarioSocialDARetHlp", "Action", null, null);
         DbCommand sqlCmd = null;
         try{
            dctx.OpenConnection(myFirm);
            sqlCmd = dctx.CreateCommand();
         } catch(Exception ex){
      throw new StandardException(MessageType.Error, "", "DocenteUsuarioSocialDARetHlp: No se pudo conectar a la base de datos", "POV.Licencias.DA", 
         "DocenteUsuarioSocialDARetHlp", "Action", null, null);
         }
         DbParameter sqlParam;
         StringBuilder sCmd = new StringBuilder();
         sCmd.Append(" SELECT DISTINCT licRef.DocenteID ");
         ...
         try{
            ...
            throw new Exception("DocenteUsuarioSocialDARetHlp: Se encontraron problemas al recuperar los datos. " + exmsg);
         } finally{
            try{ dctx.CloseConnection(myFirm); } catch(Exception){ }
         }
         return ds;
